...
void DeformableCCD::add_collision_geometry(CollisionGeometryPtr cg)
{
  CollisionDetection::add_collision_geometry(cg);
  _rebuild_bounds_vecs = true;
}

void DeformableCCD::add_rigid_body(RigidBodyPtr rb)
{
  CollisionDetection::add_rigid_body(rb);
  _rebuild_bounds_vecs = true;
}

void DeformableCCD::add_deformable_body(DeformableBodyPtr db)
{
  CollisionDetection::add_deformable_body(db);
  _rebuild_bounds_vecs = true;
}

void DeformableCCD::add_articulated_body(ArticulatedBodyPtr abody, bool disable_adjacent)
{
  CollisionDetection::add_articulated_body(abody, disable_adjacent);
  _rebuild_bounds_vecs = true;
}

void DeformableCCD::remove_collision_geometry(CollisionGeometryPtr cg)
{
  CollisionDetection::remove_collision_geometry(cg);
  _rebuild_bounds_vecs = true;
}

void DeformableCCD::remove_all_collision_geometries()
{
  CollisionDetection::remove_all_collision_geometries();
  _rebuild_bounds_vecs = true;
}

void DeformableCCD::remove_rigid_body(RigidBodyPtr rb)
{
  CollisionDetection::remove_rigid_body(rb);
  _rebuild_bounds_vecs = true;
}

void DeformableCCD::remove_articulated_body(ArticulatedBodyPtr abody)
{
...
map<SingleBodyPtr, pair<Vector3, Vector3> > DeformableCCD::get_velocities(const vector<pair<DynamicBodyPtr, VectorN> >& q0, const vector<pair<DynamicBodyPtr, VectorN> >& q1, double dt) const
{
 ...
 */
bool DeformableCCD::is_contact(double dt, const vector<pair<DynamicBodyPtr, VectorN> >& q0, const vector<pair<DynamicBodyPtr, VectorN> >& q1, vector<Event>& contacts)
{
  DStruct ds;
  ...
  FILE_LOG(LOG_COLDET) << "DeformableCCD::is_contact() entered" << endl;
  ...
  FILE_LOG(LOG_COLDET) << "DeformableCCD::is_contact() exited" << endl << endl;
 ...
 */
void DeformableCCD::check_geoms(double dt, CollisionGeometryPtr a, CollisionGeometryPtr b, const Matrix4& aTb, const Matrix4& bTa, const pair<Vector3, Vector3>& a_vel, const pair<Vector3, Vector3>& b_vel, vector<Event>& contacts)
{
  ...
  while (!q.empty())
  {
    ...
    // NOTE: we test whether the two BVs are equal first to preclude checking
    // the same BVs for intersection
    if (ax != bx && !BV::intersects(bva, bvb, aTb))
    {
  ...
  {
    // insert the contacts from these geometries into set of all contacts
  ...
  FILE_LOG(LOG_COLDET) << "DeformableCCD::check_geoms() exited" << endl;
}

/// Checks a set of vertices of geometry a against geometry b
void DeformableCCD::check_vertices(double dt, CollisionGeometryPtr a, CollisionGeometryPtr b, BVPtr bvb, const std::vector<const Vector3*>& a_verts, const Matrix4& bTa, const pair<Vector3, Vector3>& a_vel, const pair<Vector3, Vector3>& b_vel, double& earliest, vector<Event>& local_contacts, bool self_check) const
{
  ...
  vector<pair<double, pair<Vector3, const Vector3*> > > Q;
  Q.clear();
...
BVPtr DeformableCCD::get_vel_exp_BV(CollisionGeometryPtr cg, BVPtr bv, const Vector3& lv, const Vector3& av)
{
  ...
  map<BVPtr, BVPtr>::const_iterator vj = vi->second.find(bv);
  if (vj != vi->second.end())
    return vj->second;
  ...
  if (rb)
    ve_bv = bv->calc_vel_exp_BV(cg, (double) 1.0, lv, av);
  else
  {
    ...
    {
      // *must* be a bounding sphere
      shared_ptr<BoundingSphere> bs = dynamic_pointer_cast<BoundingSphere>(bv);
      assert(bs);
      shared_ptr<BoundingSphere> nbs(new BoundingSphere(*bs));
      nbs->radius += max_mvmt;
      ve_bv = nbs;
    }
  }

  // output the new bounding volume
  FILE_LOG(LOG_BV) << "new BV: " << ve_bv << std::endl;
...
Event DeformableCCD::create_contact(double toi, CollisionGeometryPtr a, CollisionGeometryPtr b, const Vector3& point, const Vector3& normal)
{
  Event e;
  e.t = toi;
...
void DeformableCCD::calc_pdot(DStruct* ds, CollisionGeometryPtr gb, CollisionGeometryPtr gs, const Vector3& bblv, const Vector3& bbav, const Vector3& bs_lvel, const Vector3& bs_avel) const
{
  ...
  RigidBodyPtr rb = dynamic_pointer_cast<RigidBody>(bb);
  ...
  // calculate pdot -- if either body is deformable we'll have to do this
  // the slow way
  if (rb && rs)
  {
 ...
 */
double DeformableCCD::determine_TOI(double t0, double tf, const DStruct* ds, Vector3& pt, Vector3& normal) const
{
  ...
  SingleBodyPtr bs = gs->get_single_body();
  DynamicBodyPtr bs_sb = bs->get_articulated_body();
  if (!bs_sb)
    bs_sb = bs;
  ...
  while (!Q.empty())
  {
    ...
    if (!ds->self_check && !BV::intersects(&O, gs_BV.get()))
    {
    ...
    else
    {
      ...
      if (gs_primitive->intersect_seg(gs_BV, LineSeg3(pa, pb), t, pt, normal))
      {
        ...
        FILE_LOG(LOG_COLDET) << "DeformableCCD::determine_TOI() exited" << endl;
 ...
 */
double DeformableCCD::calc_min_dev(const Vector3& u, const Vector3& d, const Quat& q1, const Quat& q2, double& t)
{
...
pair<double, double> DeformableCCD::calc_deviations(const Vector3& u, const Vector3& d, const Quat& q1, const Quat& q2, double t1, double t2)
{
 ...
 */
double DeformableCCD::calc_max_dev(const Vector3& u, const Vector3& d, const Quat& q1, const Quat& q2, double& t)
{
...
void DeformableCCD::broad_phase(const map<SingleBodyPtr, pair<Vector3, Vector3> >& vel_map, vector<pair<CollisionGeometryPtr, CollisionGeometryPtr> >& to_check)
{
  ...
  for (map<sorted_pair<CollisionGeometryPtr>, unsigned>::const_iterator i = overlaps.begin(); i != overlaps.end(); i++)
  {
    ...
    // get the single bodies corresponding to the geometries
    SingleBodyPtr sb1 = i->first.first->get_single_body();
    SingleBodyPtr sb2 = i->first.second->get_single_body();

    // attempt to case the single bodies as rigid bodies 
    ...
    // if both are rigid bodies and are disabled, don't check
    if (rb1 && rb2 && !rb1->is_enabled() && !rb2->is_enabled())
      continue;
...
void DeformableCCD::sort_AABBs(const map<SingleBodyPtr, pair<Vector3, Vector3> >& vel_map)
{
  ...
  else
  {
    ...
    for (unsigned i=0; i< _x_bounds.size(); i++)
    {
      CollisionGeometryPtr geom = _x_bounds[i].second.geom;
...
void DeformableCCD::update_bounds_vector(vector<pair<double, BoundsStruct> >& bounds, const map<SingleBodyPtr, pair<Vector3, Vector3> >& vel_map, AxisType axis)
{
  const unsigned X = 0, Y = 1, Z = 2;
  BVPtr bv_exp;
  ...
  for (unsigned i=0; i< bounds.size(); i++)
  {
    ...
    // two cases: rigid body and deformable body
    ...
    else
    {
      ...
      {
        // make a new, expanded bounding sphere
        shared_ptr<BoundingSphere> bs = dynamic_pointer_cast<BoundingSphere>(bv);
        assert(bs);
        shared_ptr<BoundingSphere> nbs(new BoundingSphere(*bs));
        nbs->radius += max_mvmt;
...
void DeformableCCD::build_bv_vector(const map<SingleBodyPtr, pair<Vector3, Vector3> >& vel_map, vector<pair<double, BoundsStruct> >& bounds)
{
  const double INF = std::numeric_limits<double>::max();
  ...
  for (set<CollisionGeometryPtr>::const_iterator i = _geoms.begin(); i != _geoms.end(); i++)
  {
    ...
    // get the single body and primitive for the geometry
    SingleBodyPtr sb = (*i)->get_single_body();
...
bool DeformableCCD::intersect_BV_trees(BVPtr a, BVPtr b, const Matrix4& aTb, CollisionGeometryPtr geom_a, CollisionGeometryPtr geom_b) 
{
  ...
  FILE_LOG(LOG_COLDET) << "DeformableCCD::intersect_BV_trees() entered" << endl;

  // intersect the BVs at the top level
  if (!BV::intersects(a, b, aTb))
  {
    FILE_LOG(LOG_COLDET) << "  no intersection at top-level BVs" << endl;
    FILE_LOG(LOG_COLDET) << "DeformableCCD::intersect_BV_trees() exited" << endl;
