...
class NaNException : public std::exception
{
  virtual const char* what() const throw()
  {
    return "An MPFR function has returned NaN.";
  }
};

class InexactException : public std::exception
{
  virtual const char* what() const throw()
  {
    return "The result of an MPFR function cannot be represented exactly and must be rounded.";
  }
};
...
class RangeException : public std::exception
{
  virtual const char* what() const throw()
  {
    return "An argument to a MPFR function was NaN.";
  }
};
...
// MPFR 2.4.0 Specifics
  ...
  mpreal eps() const { unsigned long prec = get_prec() - 1; mpreal one((unsigned long) 1); return one.operator>>=(prec); } 
  void    set_prec(mp_prec_t prec, mp_rnd_t rnd_mode = default_rnd);  // Change precision with rounding mode
  
  // Set mpreal to +-inf, NaN
  mpreal&      set_inf(int sign = +1);  
  mpreal&    set_nan();
  ...
  static mpreal inf() { mpreal x; return x.set_inf(1); }

  /// checks for exception flags being set and throws appropriate exception if necessary
  static void check()
  {
/*
    if (mpfr_underflow_p()) { throw UnderflowException(); mpfr_clear_underflow(); }
    if (mpfr_overflow_p()) { throw OverflowException(); mpfr_clear_overflow(); }
    if (mpfr_nanflag_p()) { throw NaNException(); mpfr_clear_nanflag(); }
    if (mpfr_inexflag_p()) { throw InexactException(); mpfr_clear_inexflag(); }
    if (mpfr_erangeflag_p()) { throw RangeException(); mpfr_clear_erangeflag(); }
*/
  }
...
inline mpreal& mpreal::set_inf(int sign) 
{ 
  mpfr_set_inf(mp,sign);
  return *this;
}  

inline mpreal& mpreal::set_nan() 
{
  mpfr_set_nan(mp);
  return *this;
}
...
namespace std
{
  ...
  inline mpfr::mpreal fabs(const mpfr::mpreal& x)
  {
    return mpfr::fabs(x);
  }
  ...
  inline mpfr::mpreal acos(const mpfr::mpreal& x)
  {
    return mpfr::acos(x);
  }
  ...
  inline mpfr::mpreal atan(const mpfr::mpreal& x)
  {
    return mpfr::atan(x);
  }

  inline mpfr::mpreal atan2(const mpfr::mpreal& y, const mpfr::mpreal& x)
  {
    return mpfr::atan2(y,x);
  }

  inline bool isinf(const mpfr::mpreal& x)
  {
    return mpfr::_isinf(x);
  }

  inline mpfr::mpreal pow(const mpfr::mpreal& base, const mpfr::mpreal& exponent)
  {
  ...
  inline bool isnan(const mpfr::mpreal& x)
  {
    return mpfr::_isnan(x);
  }

  inline mpfr::mpreal asin(const mpfr::mpreal& x)
  {
    return mpfr::asin(x);
  }
...
public:
   ...
   static mpfr::mpreal min() 
   {
     static bool first_time = true;
     static mpfr::mpreal minimum;
     if (first_time)
     {
       mpfr_clear_underflow();
       first_time = false;
       minimum = (mpfr::mpreal) 1.0;
       mpfr::mpreal tmp;
       while (true)
       {
         tmp = minimum >> 1;
   ...
   static mpfr::mpreal max() 
   {
     static bool first_time = true;
     static mpfr::mpreal maximum;
     if (first_time)
     {
       mpfr_clear_overflow();
       first_time = false;
       maximum = (mpfr::mpreal) 1.0;
       mpfr::mpreal tmp;
       while (true)
       {
         tmp = maximum << 1;
   ...
   // should be correct, or very close to it
   static mpfr::mpreal epsilon() 
   {
     static bool first_time = true;
     static mpfr::mpreal eps;
     if (first_time)
     {
       first_time = false;
       eps = (mpfr::mpreal) 2.0 >> (mpfr::mpreal::default_prec - 1);
     }
     return eps;
   }

   static mpfr::mpreal round_error() {
     return static_cast<mpfr::mpreal>(0);
   }

   // correct
   static mpfr::mpreal infinity() {
     mpfr::mpreal x;
     x.set_inf();
     return x;
   }
   ...
   // correct?
   static mpfr::mpreal quiet_NaN() {
     return static_cast<mpfr::mpreal>(0);
   }

   // correct?
   static mpfr::mpreal denorm_min() {
     return static_cast<mpfr::mpreal>(0);
   }
  
}; // numeric_limits (specialized)
} // namespace std
#endif /* __MP_REAL_H__ */
