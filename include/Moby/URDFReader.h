...
{
  public:
    static std::map<std::string, std::pair<std::vector<RigidBodyPtr>, std::vector<JointPtr> > > read(const std::string& fname);
  ...
  private:
    ...
    static void read_robot(XMLTreeConstPtr node, URDFData& data, std::map<std::string, std::pair<std::vector<RigidBodyPtr>, std::vector<JointPtr> > >& robot_map); 
}; // end class
} // end namespace
