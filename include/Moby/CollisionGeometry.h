...
class CollisionGeometry : public virtual Base
{
  public:
    CollisionGeometry();
    void set_transform(const Ravelin::Pose3d& transform, bool rel_transform_accounted);  
    void write_vrml(const std::string& filename) const;
    PrimitivePtr set_geometry(PrimitivePtr primitive);
    void set_rel_transform(const Ravelin::Pose3d& transform, bool update_global_transform = false);
    ...
    boost::shared_ptr<const Ravelin::Pose3d> get_transform() const { return _transform; }

    /// Gets the relative transform for this CollisionGeometry
    boost::shared_ptr<const Ravelin::Pose3d> get_rel_transform() const { return _rel_transform; }
  ...
  protected:
    ...
    boost::shared_ptr<Ravelin::Pose3d> _transform;
  
    /// The relative transform to the CollisionGeometry frame
    boost::shared_ptr<Ravelin::Pose3d> _rel_transform;
