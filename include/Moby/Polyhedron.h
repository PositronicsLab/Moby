...
class Polyhedron
{
  public:
    ...
    bool inside_or_on(const Ravelin::Point3d& point, double tol = NEAR_ZERO);
    LocationType location(const Ravelin::Point3d& point, double tol = NEAR_ZERO) const;
    static void to_vrml(std::ostream& out, const Polyhedron& p, Ravelin::Point3d diffuse_color = Ravelin::Point3d(1,1,1), bool wireframe = false);
    ...
    double calc_signed_distance(const Ravelin::Point3d& point, unsigned& closest_facet);

    /// Gets the signed distance from a point to the polyhedron
    double calc_signed_distance(const Ravelin::Point3d& point) { unsigned discard; return calc_signed_distance(point, discard); }

    /// Gets the corners of the axis-aligned bounding box of this polyhedron
    std::pair<Ravelin::Point3d, Ravelin::Point3d> get_bounding_box_corners() const { return std::make_pair(_bb_min, _bb_max); }
  ...
  private:
    static void replace_edge(const std::vector<Ravelin::Point3d>& v, std::vector<IndexedTri>& f, unsigned a, unsigned b, unsigned c, std::vector<unsigned>& del_list);
    static bool find_vertex(const std::vector<Ravelin::Point3d>& vertices, const Ravelin::Point3d& v);
    ...
    static Ravelin::Point3d intersect_plane(const Ravelin::Vector3d& normal, double d, const Ravelin::Point3d& p1, const Ravelin::Point3d& p2);  
    ...
    static bool bisect(const Triangle& tbi, std::vector<Ravelin::Point3d>& v, std::vector<IndexedTri>& f, unsigned i);
    static unsigned add_vertex(std::vector<Ravelin::Point3d>& vertices, const Ravelin::Point3d& v);

    Ravelin::Point3d _bb_min, _bb_max;
