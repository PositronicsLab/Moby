...
class BoundingSphere : public BV 
{
  public:
    ...
    virtual Ravelin::Point3d get_lower_bounds(const Ravelin::Pose3d& T) { return T.x+Ravelin::Point3d(center[0]-radius, center[1]-radius, center[2] - radius); }

    /// Gets the upper bounds on the bounding sphere
    virtual Ravelin::Point3d get_upper_bounds(const Ravelin::Pose3d& T) { return T.x+Ravelin::Point3d(center[0]+radius, center[1]+radius, center[2] + radius); }
