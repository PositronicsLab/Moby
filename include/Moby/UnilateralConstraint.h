...
class UnilateralConstraint 
{
  public:
    ...
    static void determine_connected_constraints(const std::vector<UnilateralConstraint>& constraints, const std::vector<JointPtr>& implicit_joints, std::list<std::list<UnilateralConstraint*> >& groups, std::list<std::list<boost::shared_ptr<Ravelin::DynamicBodyd> > >& remaining_islands);
